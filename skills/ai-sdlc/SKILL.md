...
1. Scaffold the artifact skeleton with `scripts/init_workflow.py <project-dir> --name "<project name>"`, or copy templates from `assets/` into an existing repo. The skeleton stays inside `iterations/` (REVIEW.md, bands.yaml, hooks, scripts, evals, gates, org) plus `workflow-graph.yaml` at the root — the skill does not create CLAUDE.md/AGENTS.md or .gitignore; keep your own repository-memory file and ignore rules.
...
3. Write `iterations/current/intent.md` from the template, commit it, and ask the product owner to accept or reject. Acceptance triggers Design. **Iteration layout (always):** one folder per iteration — `iterations/current/` holds the live `intent.md`, `spec.md`, and `plan.md`; when the iteration's PR merges, rename the whole folder to `iterations/NNN-<short-sha>-<slug>/` (sequence number keeps sort order, short SHA is the merge commit, e.g. `iterations/001-4f26f9f-claims-portal/`), then start the next draft at `iterations/current/intent.md`. Never invent new live paths like `iterations/002/` while it is the active iteration: the graph, `check`, and `preflight` track only `iterations/current/` and will not see anything else.

If the user is already inside a later phase (for example, "review this PR" or
"diagnose this incident"), start at that phase instead.
...
The scaffold script copies the core skeleton into a new project — everything
under `iterations/` (`iterations/current/intent.md`, `iterations/REVIEW.md`,
`iterations/bands.yaml`, `production-gate.sh`, evals, gates README, tool
scripts, and `iterations/org/...` from `init_org.py`), with the single
exception of `workflow-graph.yaml`, which stays at the repo root. The skill
does **not** create `CLAUDE.md`/`AGENTS.md` or `.gitignore`; copy these manually when extending an existing repo. `spec.md` and `plan.md`
...
- `assets/iterations-README.md` — `iterations/` directory map and workflow-graph.yaml guide (copied into new projects as `iterations/README.md`)
...
- `assets/REVIEW.md` — review standards for the AI PR-review loop (soft rules only: passes, evidence, severity, 5-nit cap; copied to `iterations/REVIEW.md`)
- `assets/bands.yaml` — monitoring control bands for Maintain (copied to `iterations/bands.yaml`)
...
- `assets/gates-README.md` — gate ledger usage (copied into new projects as `iterations/gates/README.md`)
...
- `assets/evals-README.md` — how to add evals (copied into new projects as `iterations/evals/README.md`)
...
- `scripts/init_workflow.py` — scaffold the artifact skeleton into a new project (`--dry-run`, `--git`)
...
- `scripts/sync_issues.py` — GitHub issue intake for the product engineering agent (`pull` open issues into `iterations/org/intake/github/`, `push` a feature ticket)
- `scripts/org_status.py` — agent busy/idle and review-queue management (`status`, `agent`, `review`) against `iterations/org/status.yaml`
- `scripts/intake.py` — form/email demand intake into `iterations/org/intake/` (`add`, `list`)

## Self-test (after installing)

1. `python3 skills/ai-sdlc/scripts/init_workflow.py /tmp/wf-demo --name "Demo" --git`
