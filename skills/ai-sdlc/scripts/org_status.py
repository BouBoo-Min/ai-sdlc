...
Maintains iterations/org/status.yaml through validated commands instead of hand edits:
...
ORG_CHART_DEFAULT = PROJECT_ROOT / "iterations" / "org" / "org-chart.yaml"
STATUS_DEFAULT = PROJECT_ROOT / "iterations" / "org" / "status.yaml"
...
def _agent_error(status: dict, role_by_name: dict, human_by_name: dict, name: str) -> str | None:
    ...
    if name not in agents:
        return f"agent {name!r} is not in iterations/org/status.yaml"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--status", default=str(STATUS_DEFAULT), help="iterations/org/status.yaml")
    parser.add_argument("--org-chart", default=str(ORG_CHART_DEFAULT), help="iterations/org/org-chart.yaml")
