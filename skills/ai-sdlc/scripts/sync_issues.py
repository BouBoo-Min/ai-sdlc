...
Usage:
    python3 sync_issues.py pull [--config iterations/org/intake/config.json] [--dry-run]
    python3 sync_issues.py push --title TITLE --body BODY [--repo OWNER/REPO]
        [--labels a,b] [--dry-run]

pull:
    Fetch open issues for the configured repo(s), write
    iterations/org/intake/github/<repo>-<number>.md for new issues, and remember the
...
DEFAULT_CONFIG = PROJECT_ROOT / "iterations" / "org" / "intake" / "config.json"
...
def _pull(config_path: Path, dry_run: bool) -> int:
    ...
    state_path = _resolve(PROJECT_ROOT, github.get("state_file", "iterations/org/intake/.state.json"))
    output_dir = _resolve(PROJECT_ROOT, github.get("output_dir", "iterations/org/intake/github"))
...
def main() -> int:
    ...
    pull = sub.add_parser("pull", help="fetch open issues into iterations/org/intake/github/")
    pull.add_argument(
        "--config",
