...
`iterations/scripts/workflow_state.py` treats `workflow-graph.yaml` as the live state
machine and `iterations/gates/ledger.jsonl` as the approval history:
...
python3 iterations/scripts/gate_ledger.py record \
  --gate product_owner_accept --artifact iterations/current/intent.md \
  --commit abc123 --approver "Ada" --evidence "intent review"
python3 iterations/scripts/workflow_state.py advance --node intent --record product_owner_accept-001
