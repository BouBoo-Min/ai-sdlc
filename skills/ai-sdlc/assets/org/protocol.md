...
2. When an agent accepts or finishes work, it updates `iterations/org/status.yaml`
...
2. The CTO assigns an idle reviewer from `iterations/org/status.yaml` and marks it busy.
   If no reviewer is idle, the request waits in the review queue.
3. The reviewer writes `iterations/org/reviews/<artifact>-<id>.md` with severity-ranked,
...
- GitHub issues: `scripts/sync_issues.py pull` → `iterations/org/intake/github/<repo>-<n>.md`
- Forms/email: `scripts/intake.py add --source form|email …` writes
  `iterations/org/intake/forms/<record>.md` / `iterations/org/intake/email/<record>.md`.
