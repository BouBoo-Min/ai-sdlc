...
if [[ "$approval" == ledger:* ]]; then
  ...
  project_dir="$(dirname "$(dirname "$hook_dir")")"  # hook lives at iterations/hooks/
  ledger_script="${GATE_LEDGER_SCRIPT:-$project_dir/iterations/scripts/gate_ledger.py}"
  ledger_file="${GATE_LEDGER_FILE:-$project_dir/iterations/gates/ledger.jsonl}"
