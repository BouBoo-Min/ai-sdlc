...
Each eval is **one JSON file** in this directory (`iterations/evals/*.json`). An eval is a
real task from recent work, its prompt, and machine-checkable pass conditions.

Run the suite locally:

```bash
python3 iterations/scripts/run_evals.py iterations/evals/ --min-pass-rate 0.9
```
...
`iterations/evals/example.md` for the markdown reference form.
